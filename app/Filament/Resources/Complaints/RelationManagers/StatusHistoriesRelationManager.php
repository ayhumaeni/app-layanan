...
class StatusHistoriesRelationManager extends RelationManager
{
    ...
    protected static ?string $title = 'Riwayat Status Penanganan Pengaduan';
        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('from_status')
                    ->label('Dari Status')
                    ->badge()
                    ->formatStateUsing(fn ($state) => $state ? (ComplaintStatus::tryFrom($state)?->label() ?? $state) : 'Awal'),
                TextColumn::make('to_status')
                    ->label('Menjadi Status')
                    ->badge()
                    ->color(fn ($state) => ComplaintStatus::tryFrom($state)?->color() ?? 'primary')
                    ->formatStateUsing(fn ($state) => ComplaintStatus::tryFrom($state)?->label() ?? $state),
